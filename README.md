...
| Symphony | Stokowski |
|----------|-----------|
| `codex app-server` JSON-RPC | `claude -p --output-format stream-json`, `codex exec --json`, or `gemini --output-format stream-json` |
| `thread/start` → thread_id | First turn → `session_id` |
| `turn/start` on thread | `claude -p --resume <session_id>` or `gemini -p --resume <session_id>` |
...
- **Native UI runner mode** — run Claude Code, Codex, or Gemini inside tmux (`runner: tmux`, `tmux:codex`, `tmux:gemini`) when you want the real CLI UI instead of headless JSON mode
- **Fallback runner chain** — on rate limit errors, automatically retry with fallback runners (e.g. `fallback_runners: [gemini, codex]`)
- **Fallback model chain** — retry rate-limited Claude/Gemini states on backup models via `fallback_models`
...
- **Session continuity** — multi-turn agent sessions via `--resume` (Claude Code and Gemini CLI); session IDs are persisted to `.stokowski/sessions.json`
...
- **Conditional gates** — gates can auto-approve immediately, auto-approve only when the agent asked no questions, or bounce back to rework when a human replies in comments
- **Comment-driven rework** — webhook comment events can instantly send a pending gate back to its `rework_to` state when a human answers agent questions
- **Persistent state** — token metrics and schedule timestamps survive restarts (JSON file next to workflow YAML)
- **Orphan agent cleanup** — kills stale headless agent processes and the shared `stokowski` tmux session on startup to prevent token bleed from prior crashes
...
- **Gate automation** — gates can auto-approve, trigger rework from PR reviews, or trigger rework when a human replies to agent questions
- **Persistent tracking** — state, run counters, token totals, and session IDs survive restarts without polluting tracker comments
...
- **Codex** — `codex exec --json` for headless runs, or `runner: tmux:codex` for the native REPL UI
- **Gemini CLI** — `gemini --output-format stream-json` with multi-turn `--resume`
- **Fallback chains** — fail over to backup models or backup runners on rate limits
...
- **Graceful shutdown** — `q` kills headless subprocesses and tmux windows before exiting, so you don't bleed tokens on orphaned agents
...
- **Orphan agent cleanup** — kills stale headless agent processes and stale tmux sessions on startup to prevent token bleed from prior crashes
...
- **Per-state overrides** — runner, model, fallback models/runners, max_turns, timeouts, hooks, session mode, and permission mode can all be overridden per state
...
claude:
  ...
  fallback_models:                     # optional fallback model chain on rate limits
    - claude-haiku-4-5
  ...
  mcp_config:                          # optional explicit MCP config files or JSON strings
    - .mcp.json
...
states:                                # the state machine pipeline
  investigate:
    ...
    linear_state: active
    runner: claude                     # "claude", "codex", "gemini", "tmux", "tmux:codex", or "tmux:gemini"
  ...
  review_investigation:
    ...
    max_rework: 3
    auto_approve: when_no_questions    # "never", "when_no_questions", or "always"
    rework_on_comment: true            # new human comments can trigger rework immediately
    pr_triggers:                       # optional PR review → gate action mapping
      approved: approve
...
|------|-----------|---------------------|
| `agent` (default) | Yes | Dispatches a runner, runs one state-machine turn, follows `transitions.complete` on success, and optionally follows `transitions.rework` when the agent emits `STOKOWSKI:REWORK` |
| `gate` | No | Moves issue to review Linear state, waits for human or PR review activity. Follows `transitions.approve` on approval, `rework_to` on Rework, and can auto-approve or auto-rework based on config |
...
| `fallback_models` | root `claude.fallback_models` | Backup models tried in order on rate limits |
| `fallback_runners` | none | Backup runners tried in order on rate limits |
...
| `max_rework` | none | For agent/gate states, max rework cycles before escalation/blocking |
| `auto_approve` | `never` | Gate-only: `never`, `when_no_questions`, or `always` |
| `rework_on_comment` | `false` | Gate-only: when true, fresh human comments can trigger immediate rework |
| `pr_triggers` | none | Gate-only: map GitHub PR events such as `approved`, `changes_requested`, `merged` to actions |
...
Agents run with `cwd` set to the workspace, so `.mcp.json` in the repo root is picked up automatically. If you want to pass explicit MCP configs in headless mode, use `claude.mcp_config`.
...
prompts/       →  Jinja2 stage prompt files
    ...
    │   └── --resume <session_id>  (multi-turn continuity)
    ├── Tmux mode: native Claude / Codex / Gemini UI in tmux windows
...
| `stokowski/tracking.py` | Tracking payload helpers and recent-comment filtering for lifecycle prompts |
...
- **`permission_mode: auto`** passes `--dangerously-skip-permissions` to Claude Code, `--approval-mode yolo` to Gemini, and `--dangerously-bypass-approvals-and-sandbox` to Codex. Only use it in trusted environments or isolated containers.
